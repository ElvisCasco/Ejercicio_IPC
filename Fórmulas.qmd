...
required = {
    "numpy": "numpy",
    "polars": "polars",
    "fastexcel": "fastexcel",          # polars Excel backend
...
# wd = "//srvfs02/IE/Investigaciones/2025/IPC_Calc/"
# wd = "C:/Trabajo/GitHub/Ejercicio_IPC-main/"

# Resolve the directory containing this .qmd file.
# - When rendered with Quarto, QUARTO_PROJECT_DIR / QUARTO_DOCUMENT_PATH are set.
# - Interactively (Jupyter / VS Code), fall back to __file__ or cwd.
import os
...
wd = str(Path(_qmd_path).resolve().parent if Path(_qmd_path).is_file()
         else Path(_qmd_path).resolve()) + "/"
```
...
def get_ponds_from_xlsx():
    ...
    df.write_parquet(wd + "data/Categorias.parquet")
    return df
...
def obtener_serie_precio_t(region):
    df = obtener_precio_t(region)
    df.transpose(
        include_header=True, header_name="Variedad", column_names=[df.columns[3:]]
    )
...
```{python}
def guardar_precios(region):
    # region = "MDC"
    #### en t-1
    df = obtener_precio_t_1(region)
    precio_unitario_t_1 = (
        df
        .unpivot(index=df.columns[:3], value_name='Precio_Unitario_t_1')
        .rename({df.columns[0]: "Establecimiento","variable":"Variedad"})
        .cast({"Precio_Unitario_t_1": pl.Float64},strict=False)
        .with_columns(Region = pl.lit(region))
    )
    # precio_unitario_t_1.write_parquet(wd + "data/" + region + "_precio_unitario_t_1.parquet")
    df = obtener_contenido_t_1(region)
    contenido_t_1 = (
        df
        .unpivot(index=df.columns[:3], value_name='Contenido_t_1')
        .rename({df.columns[0]: "Establecimiento","variable":"Variedad"})
        .cast({"Contenido_t_1": pl.Float64},strict=False)
    )
    # contenido_t_1.write_parquet(wd  + "data/" + region + "_contenido_t_1.parquet")
    precio_t_1 = (
        pl.concat([
            pl.DataFrame(precio_unitario_t_1["Region","Codigo","Establecimiento","Variedad","Precio_Unitario_t_1"]),
            pl.DataFrame(contenido_t_1["Contenido_t_1"])], 
            how="horizontal")
        # .with_columns(
            # (pl.col("Precio_Unitario_t_1") / pl.col("Contenido_t_1")).alias('Precio_t_1'))
    )
    # precio_t_1.write_parquet(wd + "data/" + region + "_precio_t_1.parquet")

    #### en t
    df = obtener_precio_t(region)
    precio_unitario_t = (
        df
        .unpivot(index=df.columns[:3], value_name='Precio_Unitario_t')
        .rename({df.columns[0]: "Establecimiento","variable":"Variedad"})
        .cast({"Precio_Unitario_t": pl.Float64},strict=False)
        .with_columns(Region = pl.lit(region))
    )
    # precio_unitario_t.write_parquet(wd + "data/" + region + "_precio_unitario_t.parquet")
    df = obtener_contenido_t(region)
    contenido_t = (
        df
        .unpivot(index=df.columns[:3], value_name='Contenido_t')
        .rename({df.columns[0]: "Establecimiento","variable":"Variedad"})
        .cast({"Contenido_t": pl.Float64},strict=False)
    )
    # contenido_t.write_parquet(wd + "data/" + region + "_contenido_t.parquet")
    precio_t = (
        pl.concat([
            pl.DataFrame(precio_unitario_t["Region","Codigo","Establecimiento","Variedad","Precio_Unitario_t"]),
            pl.DataFrame(contenido_t["Contenido_t"])], 
            how="horizontal")
        # .with_columns(
        #     (pl.col("Precio_Unitario_t") / pl.col("Contenido_t")).alias('Precio_t'))
    )
    # precio_t.write_parquet(wd + "data/" + region + "_precio_t.parquet")

    precios = (
        pl.concat([
            pl.DataFrame(precio_t_1[
                "Region","Codigo","Establecimiento","Variedad",
                "Precio_Unitario_t_1","Contenido_t_1"]),#,"Precio_t_1"]),
            pl.DataFrame(precio_t["Precio_Unitario_t","Contenido_t"])],#,"Precio_t"])], 
            how="horizontal")
    )
    # precios.write_parquet(wd + "data/" + region + "_precios.parquet")
    return precios
```

```{python}
# Serie de precios e índices, por región, establecimietno y variedad
regions = ["MDC","RUC","MSPS","RUN","ULA","UOri","UOcc","US"]
precios_all = pl.DataFrame(schema={
    "Region":str, 
    "Codigo":str, 
    "Establecimiento":str, 
    "Variedad":str, 
    "Precio_Unitario_t_1":float,
    "Contenido_t_1":float,
    # "Precio_t_1":float,
    "Precio_Unitario_t":float,
    "Contenido_t":float,
    # "Precio_t":float,
    })
for i, region in enumerate(regions):
    precios = guardar_precios(region)
    precios_all = (
        pl.concat([
            pl.DataFrame(precios_all),
            pl.DataFrame(precios)], 
            how="vertical")
    )
...
#     precios_all
#     .with_columns(
#         (pl.col("Precio_t") / pl.col("Precio_t_1")).alias('I_c'))
# )
precios_all.write_parquet(wd + "data/precios_all.parquet")
precios_all.write_csv(wd + "data/precios_all.csv")
```

# Resultados usando DuckDB

```{python}
import duckdb
conn = duckdb.connect(database=':memory:')
# Calcular precios e índices, por variedad y establecimiento
conn.execute(
    """
    CREATE OR REPLACE TABLE variedades AS
    SELECT *,
    -- Precios = Precio_Unitario * Contenido
    Precio_Unitario_t_1 / Contenido_t_1 AS Precio_t_1,
    Precio_Unitario_t / Contenido_t AS Precio_t,
    -- I_c = Precio_t / Preci_t_1
    Precio_t / Precio_t_1 AS I_c,
    FROM "data/precios_all.parquet"
    """)
...
```{python}
# Índices por variedad, media geométrica
conn.execute(
    """
    CREATE OR REPLACE TABLE indices_variedades AS
    SELECT Region,Variedad,--,Codigo,
    array_slice(Variedad,1,8) AS Código,
    geomean(I_c) * 100 AS I_v,
    FROM variedades
    GROUP BY Region,Variedad--,Codigo
    ORDER BY Region,Variedad--,Codigo
    """)
# Guardar en archivo .parquet
conn.execute(
    """
    COPY
        (SELECT * 
        FROM indices_variedades)
        TO 'data/indices_variedades.parquet'
        (FORMAT 'parquet');
    FROM indices_variedades
    """).pl()
conn.execute(
    ...
    COPY
        (SELECT * 
        FROM indices_variedades)
        TO 'data/indices_variedades.csv'
        (FORMAT 'csv');
    FROM indices_variedades
    """).pl()
```
...
```{python}
# Ponderaciones por productos
conn.execute(
    """
    INSTALL spatial;
    LOAD spatial;
    CREATE OR REPLACE TABLE ponderaciones AS
    SELECT Estructura,Código,CCIF,Región AS Region,Pond_IPC
    FROM st_read(
        'Categorias.xlsx',
        layer = 'IPC_Calc',
...
```{python}
# Índices por producto, media geométrica
conn.execute(
    """
    CREATE OR REPLACE TABLE indices_producto AS
    SELECT Region,Código,--,Codigo,
    geomean(I_v) AS I_p,
    array_slice(Código,1,2) AS Division,
    array_slice(Código,1,3) AS Grupo,
    array_slice(Código,1,4) AS Clase,
    array_slice(Código,1,5) AS SubClase,
    array_slice(Código,1,6) AS Categoria,
    FROM indices_variedades
    GROUP BY Region,Código--,Codigo
    ORDER BY Region,Código--,Codigo
    """)
conn.execute(
    """
    CREATE OR REPLACE TABLE indices_producto AS
...
conn.execute(
    ...
    COPY
        (SELECT * 
        FROM indices_producto)
        TO 'data/indices_producto.parquet'
        (FORMAT 'parquet');
    FROM indices_producto
    ORDER BY Código,Region
    """).pl()
conn.execute(
    """
    COPY
...
conn.execute(
    """
    CREATE OR REPLACE TABLE indices_g_producto AS
    SELECT Código,
    sum(I_p * Pond_IPC) / sum(Pond_IPC) AS I_b,
    sum(Pond_IPC) AS w_b,
    FROM indices_producto
    GROUP BY Código
    ORDER BY Código
...
conn.execute(
    ...
    COPY
        (SELECT * 
        FROM indices_g_producto)
        TO 'data/indices_g_producto.parquet'
        (FORMAT 'parquet');
    FROM indices_g_producto
    """).pl()
conn.execute(
    ...
    COPY
        (SELECT * 
        FROM indices_g_producto)
        TO 'data/indices_g_producto.csv'
        (FORMAT 'csv');
    FROM indices_g_producto
    """).pl()
```
...
conn.execute(
    """
    CREATE OR REPLACE TABLE indices_g_Region AS
    SELECT Region,
    sum(I_p * Pond_IPC) / sum(Pond_IPC) AS I_b,
    sum(Pond_IPC) AS w_b,
    FROM indices_producto
...
conn.execute(
    ...
    COPY
        (SELECT * 
        FROM indices_g_Region)
        TO 'data/indices_g_Region.parquet'
        (FORMAT 'parquet');
    FROM indices_g_Region
    """).pl()
conn.execute(
...
conn.execute(
    """
    CREATE OR REPLACE TABLE indices_g_Division AS
    SELECT Division,
    sum(I_p * Pond_IPC) / sum(Pond_IPC) AS I_b,
    sum(Pond_IPC) AS w_b,
    FROM indices_producto
...
conn.execute(
    ...
    COPY
        (SELECT * 
        FROM indices_g_Division)
        TO 'data/indices_g_Division.parquet'
        (FORMAT 'parquet');
    FROM indices_g_Division
    """).pl()
conn.execute(
    ...
    COPY
        (SELECT * 
        FROM indices_g_Division)
        TO 'data/indices_g_Division.csv'
        (FORMAT 'csv');
    FROM indices_g_Division
    """).pl()
```
...
conn.execute(
    """
    CREATE OR REPLACE TABLE indices_g_Grupo AS
    SELECT Grupo,
    sum(I_p * Pond_IPC) / sum(Pond_IPC) AS I_b,
    sum(Pond_IPC) AS w_b,
    FROM indices_producto
...
conn.execute(
    ...
    COPY
        (SELECT * 
        FROM indices_g_Grupo)
        TO 'data/indices_g_Grupo.parquet'
        (FORMAT 'parquet');
    FROM indices_g_Grupo
    """).pl()
conn.execute(

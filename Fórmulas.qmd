...
def guardar_parquet(region):
    ...
    precio_t = (
        pl.concat([
            ...
            how="horizontal")
        .with_columns((pl.col("Precio_Unitario_t") * pl.col("Contenido_t")).alias('Precio_t'))
    )
...
for i, region in enumerate(regions):
    precios = guardar_parquet(region)
    precios_all = (
        pl.concat([
...
precios_all = (
    precios_all
    .with_columns(
        (pl.col("Precio_t") / pl.col("Precio_t_1")).alias('I_v'))
)
...
result = conn.execute(
    ...
    SELECT *,
    FROM "//srvfs02/IE/Investigaciones/2025/IPC_Calc/data/precios_all.parquet"
    """).pl()
result
```
